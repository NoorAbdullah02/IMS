export const renderDeptHeadUsers = (users) => {
    ...
    <div class="space-y-8 animate-fadeIn">
        ...
        <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-[2rem] shadow-2xl border-2 border-white/5 overflow-hidden w-full">
            <table class="w-full">
                <thead>
                    <tr class="bg-white/5">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-6">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">${card.examName}</p>
                                </div>
                            </div>
                            <div class="text-right flex items-center justify-end space-x-3">
                                <span class="inline-flex items-center px-4 py-1.5 rounded-full text-[10px] font-black bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase tracking-widest mr-4">
                                    <span class="w-2 h-2 rounded-full bg-emerald-500 mr-2 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></span>
                                    ${card.status}
                                </span>
                                
                                <button onclick="window.editAdmitCard(${card.id}, '${card.examName}', '${card.status}')" class="p-2 bg-slate-800 rounded-xl border border-white/10 hover:border-indigo-500/50 hover:text-indigo-400 transition-all text-slate-400">
                                    <ion-icon name="create-outline"></ion-icon>
                                </button>
                                <button onclick="window.viewAdmitCard('${card.fileUrl}')" class="p-2 bg-slate-800 rounded-xl border border-white/10 hover:border-indigo-500/50 hover:text-indigo-400 transition-all text-slate-400">
        ...
        <div id="editCardModal" class="hidden fixed inset-0 bg-slate-950/80 backdrop-blur-sm overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-8 border-2 border-white/5 w-[28rem] shadow-2xl rounded-[2.5rem] bg-gradient-to-br from-slate-800 to-slate-900 transform transition-all duration-300 scale-95 opacity-0 modal-content">
                ...
                <form id="editCardForm" class="space-y-6">
                    ...
                    <div class="flex gap-4 pt-4">
                        <button type="button" onclick="window.hideEditCardModal()" class="flex-1 px-6 py-4 rounded-xl text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-white transition-all">Cancel</button>
                        <button type="submit" class="flex-1 px-6 py-4 rounded-2xl bg-indigo-500 text-white font-black uppercase tracking-widest text-[10px] hover:bg-indigo-600 shadow-xl shadow-indigo-500/20 transition-all">Update</button>
                    </div>
                </form>
            </div>
        </div>
...
// Edit/View Logic
window.editAdmitCard = (id, examName, status) => {
...
window.viewAdmitCard = (url) => {
    ...
    if (window.triggerSecureDownload) {
        ...
        const fullUrl = window.getDownloadUrl ? window.getDownloadUrl(url) : (url.startsWith('http') ? url : `${import.meta.env.VITE_API_URL}${url}`);
